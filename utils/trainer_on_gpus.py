...
def train_and_evaluate(model, model_name, criterion, C, lr, lambda_reg, alpha, subgradient_step, w0, r, first_best_indices,
    ...
    for epoch in range(n_epochs):
        ...
        if local_rank == 0 and (epoch % 5 == 0 or epoch == n_epochs - 1):
            # --- 0) Ensure all ranks have completed training for this epoch ---
            if device.type == "cuda":
            ...
            if dist.is_available() and dist.is_initialized():
                if dist.get_backend() == "nccl" and device.type == "cuda":
                    dist.barrier(device_ids=[device.index if device.index is not None else local_rank])
                else:
                    dist.barrier()
            ...
            # --- 5) Build model_quantized on CPU ---
            t0 = time.time()
