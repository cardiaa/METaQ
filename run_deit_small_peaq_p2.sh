...
# test_267: DeiT-Small, PRESTO con duale relativo, punto 2 (meta' della dose).
#
...
if [ -e "$LOG_FILE" ]; then
    echo "Leonardo_test_267.log esiste gia': rifiuto di sovrascriverlo." >&2
    exit 1
fi
export LOG_FILE
...
srun --ntasks=$SLURM_NTASKS --ntasks-per-node=1 bash -lc '
    ...
    --entropy_coeff 7.5e-9 \
    --sparsity_coeff 2.5e-8 \
    --perspective Y \
    --flat_schedule N \
    --mag_prune_ratio 0 \
    --quantization Y \
