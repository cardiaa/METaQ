...
# test_275: ResNet-18, pipeline contro congiunta -- direzione A stadio 2, solo entropia.
# Coefficienti (T1,T2,T3) = (1e-5, 0, 3e-8) su venti epoche.
# Parte dal checkpoint del test_273. Il confronto e' contro il test_235.
#
...
if [ -e "$LOG_FILE" ]; then
    echo "Leonardo_test_275.log esiste gia': rifiuto di sovrascriverlo." >&2
    exit 1
fi
export LOG_FILE
...
srun --ntasks=$SLURM_NTASKS --ntasks-per-node=1 bash -lc '
    ...
    --check_ddp_sync \
    --pretrained Y \
    --pretrained_checkpoint $WORK/acardia0/METaQCheckpoints/ResNet-18/pipeA_stage1_t2.pt \
    > "$OUTPUT_TARGET" 2>&1
'
